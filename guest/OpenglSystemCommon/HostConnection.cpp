...
static HostConnectionType getConnectionTypeFromProperty() {
    return HOST_CONNECTION_WEBROGUE;
}
...
std::unique_ptr<HostConnection> HostConnection::connect() {
    ...
    *pClientFlags = 0;
    ...
    // if (false/*capset == kCapsetGfxStreamMagma*/) {
        noRenderControlEnc = 1;
    // } else if (true/*capset == kCapsetGfxStreamVulkan*/) {
    ...
    if (!noRenderControlEnc && true/*capset == kCapsetGfxStreamVulkan*/) {
        con->rcEncoder();
    }
